...
public final class EMCMappers
{
    ...
    public static void map()
    {
        ...
        if (ConfigHelper.getBoolean(config, "dumpEverythingToFile", "general", false,"Want to take a look at the internals of EMC Calculation? Enable this to write all the conversions and setValue-Commands to config/ProjectE/mappingdump.json")) {
            mappingCollector = new DumpToFileCollector<>(new File(ProjectE.CONFIG_DIR, "mappingdump.json"), mappingCollector);
        }

        boolean shouldUsePregenerated = ConfigHelper.getBoolean(config,"pregenerate", "general", false, "When the next EMC mapping occurs write the results to config/ProjectE/pregenerated_emc.json and only ever run the mapping again" +
        ...
        else
        {
            ...
            SimpleGraphMapper.setLogFoundExploits(ConfigHelper.getBoolean(config, "logEMCExploits", "general", true,
            ...
            for (EMCMapper<NormalizedSimpleStack, Long> emcMapper : emcMappers)
            {
                try
                {
                    if (ConfigHelper.getBoolean(config, emcMapper.getName(), "enabledMappers", emcMapper.isAvailable(), emcMapper.getDescription()) && emcMapper.isAvailable())
                    {
                        DumpToFileCollector.currentGroupName = emcMapper.getName();

                        String subConfigPath = "mapperConfigurations." + emcMapper.getName();

                        CommentedConfig mapperConfig = config.getOptional(subConfigPath)
                           .filter(c -> c instanceof CommentedConfig)
                           .map(c -> (CommentedConfig) c)
                           .orElseGet(() -> {
                               CommentedConfig newSub = config.createSubConfig();
                               config.set(subConfigPath, newSub);
                               return newSub;
                           });

                        emcMapper.addMappings(mappingCollector, mapperConfig);
