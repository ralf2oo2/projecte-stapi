...
import net.ralf2oo2.projecte.emc.collector.MappingCollectorImpl;
...
public class SimpleGraphMapper<T, V extends Comparable<V>, A extends ValueArithmetic<V>> extends MappingCollectorImpl<T, V, A> implements ValueGenerator<T, V>
{
            ...
            for (Map.Entry<T, Set<Conversion>> entry : conversionsFor.entrySet()) {
                ...
                for (Conversion conversion : entry.getValue()) {
                    ...
                    if (ZERO.compareTo(ingredientValue) < 0 && resultValueConversion.compareTo(resultValueActual) < 0) {
                        if (overwriteConversion.containsKey(conversion.output) && overwriteConversion.get(conversion.output) != conversion) {
                            if (logFoundExploits)
                                ProjectE.LOGGER.warn("EMC Exploit: \"{}\" ingredient cost: {} value of result: {} setValueFromConversion: {}", conversion, ingredientValue, resultValueActual, overwriteConversion.get(conversion.output));
                            ...
                            ProjectE.LOGGER.warn("EMC Exploit: ingredients ({}) cost {} but output value is {}", conversion, ingredientValue, resultValueActual);
                        }
                    }
                }
