...
public abstract class MappingCollectorImpl<T, V extends Comparable<V>,  A extends ValueArithmetic<V>> extends AbstractMappingCollector<T, V, A>  {
    private static final boolean DEBUG_GRAPHMAPPER = false;

    protected final A arithmetic;
    protected MappingCollectorImpl(A arithmetic) {
        super(arithmetic);
    ...
    public void setValueBefore(T something, V value) {
        ...
        if (fixValueBeforeInherit.containsKey(something))
            ProjectE.LOGGER.debug("Overwriting fixValueBeforeInherit for {}:{} to {}", something, fixValueBeforeInherit.get(something), value);
    ...
    public void setValueAfter(T something, V value) {
        ...
        if (fixValueAfterInherit.containsKey(something))
            ProjectE.LOGGER.debug("Overwriting fixValueAfterInherit for {}:{} to {}", something, fixValueAfterInherit.get(something), value);
    ...
    public void setValueFromConversion(int outnumber, T something, Map<T, Integer> ingredientsWithAmount)
    {
        ...
        if (overwriteConversion.containsKey(something)) {
            ...
            ProjectE.LOGGER.debug("Overwriting setValueFromConversion {} with {}", overwriteConversion.get(something), conversion);
    ...
    protected class Conversion {
        ...
        public boolean equals(Object o) {
            if (!(o instanceof MappingCollectorImpl.Conversion))
                return false;
