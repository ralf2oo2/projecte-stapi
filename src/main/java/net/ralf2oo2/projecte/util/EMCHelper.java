...
public final class EMCHelper
{
    ...
    public static boolean doesItemHaveEmc(ItemStack stack)
    {
        ...
        return EMCMappers.mapContains(iStack);
    }
    ...
    public static long getEmcValue(Block block)
    {
        ...
        if (stack.isValid() && EMCMappers.mapContains(stack))
        {
            return EMCMappers.getEmcValue(stack);
        }
    ...
    public static long getEmcValue(ItemStack stack)
    {
        ...
        if (!EMCMappers.mapContains(iStack) && ItemHelper.isDamageable(stack))
        {
            ...
            if (EMCMappers.mapContains(iStack))
            {
                long emc = EMCMappers.getEmcValue(iStack);
...
//                result += getEnchantEmcBonus(stack);
        ...
        else
        {
            if (EMCMappers.mapContains(iStack))
            {
                return EMCMappers.getEmcValue(iStack) + getStoredEMCBonus(stack);
            }
        }
    ...
    public static long getEmcSellValue(ItemStack stack)
    {
        ...
        long emc = (long) Math.floor(originalValue * EMCMappers.covalenceLoss);

        if (emc < 1)
        {
            if (EMCMappers.covalenceLossRounding)
            {
                emc = 1;
            }
            else
    ...
    public static long getStackEmc(ItemStack stack)
    {
        if (StackUtil.isEmpty(stack))
        {
            return 0;
        ...
        try
        {
            return Math.multiplyExact(value, (long) stack.count);
        }
    ...
    {
        if (EMCMappers.covalenceLoss == 1.0)
        {
            return " ";
        }
    ...
    private static long getStoredEMCBonus(ItemStack stack) {
        if (stack.getStationNbt().contains("StoredEMC")) {
            return stack.getStationNbt().getLong("StoredEMC");
        } else if (stack.getItem() instanceof ItemEmc itemEmc) {
            return itemEmc.getStoredEmc(stack);
        }
        return 0;
    }
    ...
    {
        double unprocessedEMC = stack.getStationNbt().getDouble("UnprocessedEMC");
        unprocessedEMC += amount;
        long toRemove = (long) unprocessedEMC;
        unprocessedEMC -= toRemove;
        stack.getStationNbt().putDouble("UnprocessedEMC", unprocessedEMC);
        return toRemove;
    }
}
