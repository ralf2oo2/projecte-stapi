...
public class EnergyCollectorMK2ScreenHandler extends EnergyCollectorMK1ScreenHandler{
    public EnergyCollectorMK2ScreenHandler(PlayerInventory playerInventory, EnergyCollectorMK2BlockEntity collector) {
    ...
    void initSlots(PlayerInventory playerInventory) {
        ...
        //Klein Star Slot
        this.addSlot(new ValidatedSlot(aux, EnergyCollectorMK2BlockEntity.UPGRADING_SLOT, 140, 58, SlotPredicates.COLLECTOR_INV));
        ...
        //Upgrade Result
        this.addSlot(new ValidatedSlot(aux, EnergyCollectorMK2BlockEntity.UPGRADE_SLOT, 140, 13, SlotPredicates.COLLECTOR_INV));

        //Upgrade Target
        this.addSlot(new GhostSlot(aux, EnergyCollectorMK2BlockEntity.LOCK_SLOT, 169, 36, SlotPredicates.COLLECTOR_LOCK));

        //Player inventory
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 9; j++)
                this.addSlot(new Slot(playerInventory, j + i * 9 + 9, 20 + j * 18, 84 + i * 18));
    ...
    public ItemStack quickMove(int slotIndex) {
        ...
        if (slotIndex <= 14)
        {
            if (!ScreenHandlerHelper.mergeItemStack(slots, stack, 15, 50, false))
            {
                return null;
            }
        }
        ...
        {
            if (!FuelMapper.isStackFuel(stack) || FuelMapper.isStackMaxFuel(stack) || !ScreenHandlerHelper.mergeItemStack(slots, stack, 1, 12, false))
            {
                return null;
            }
        }
