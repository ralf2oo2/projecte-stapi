...
public class AlchemicalBagScreenHandler extends ScreenHandler {
    private final int blocked;
    private final boolean immutable;

    public AlchemicalBagScreenHandler(PlayerInventory playerInventory, Inventory bagInventory) {
        this(playerInventory, bagInventory, false);
    }

    public AlchemicalBagScreenHandler(PlayerInventory playerInventory, Inventory bagInventory, boolean immutable) {
        this.immutable = immutable;

        for (int i = 0; i < 8; i++)
            for (int j = 0; j < 13; j++)
                this.addSlot(new Slot(bagInventory, j + i * 13, 12 + j * 18, 5 + i * 18));
